...
v19 changes from v18 (Phase 9.5 build-time corrections and clarifications): (1) **The "Outbox pattern" §13 already referenced for confirm_hold/release_hold retries didn't actually exist anywhere in the codebase** -- `LoggingEventPublisher` is a no-op, and the same pattern's earlier mention (event-publish-after-confirm, §13) was never built either. Confirmed with user: built a real, minimal Outbox for this phase rather than deferring it again -- a `pending_theatre_call` table (`call_type`, `booking_id`, `theatre_hold_id`, `status`, `attempts`, `next_attempt_at`, `last_error`) written in the same transaction as the booking confirm/cancel/sweep-expiry, plus `theatre_outbox_relay.py`, structured identically to `reconciliation_sweep.py` (N replicas, one active via a distinct Postgres advisory lock, exponential backoff, FAILED after bounded attempts exhausted -- §13: "flag for manual reconciliation, an ops concern, not a data-integrity one"). (2) **Testing the live HTTP-level failure paths** (hold conflict -> 409, hold timeout -> 503) needs `MockTheatreIntegration` to actually fail on demand against the *real running* booking process -- every test in this codebase hits real services over HTTP rather than mocking, and there's no real theatre API here to fail for real. Resolved the same way: `MockTheatreIntegration`'s `hold_mode` is env-var-controlled (`THEATRE_MOCK_HOLD_MODE`), and the test suite automates stopping/restarting the real booking process with it set -- the same technique Phase 6's tests already use for the sweep workers, not a manual operator step like Phase 5's payment-down test. `confirm_hold_should_fail`/`release_hold_should_fail` (for testing the Outbox relay's retry path) are plain constructor args instead, set by tests that construct `TheatreOutboxRelayWorker` directly against the real `booking_db`, same technique Phase 6 uses for `ReconciliationSweepWorker`. (3) A real circuit breaker already existed for the payment client (§7/§13) -- extracted into its own module (`adapters/circuit_breaker.py`, parametrized on which exception type trips it) and reused for the theatre integration's own breaker, rather than duplicating the class. `payment_client.py` keeps a thin subclass under the same name defaulting to its original exception type, so the pre-existing payment circuit-breaker test needed zero changes -- this codebase's existing convention is duplicating logic *across* services (e.g. `_derive_idempotency_key`), not within one service's own adapters directory, where sharing the real class is the better call. (4) A real Python gotcha surfaced while wiring up the first standalone workers ever to import sibling modules (`theatre_outbox_relay.py`/`theatre_availability_sync.py` importing `adapters.X`/`domain.X`, unlike `reconciliation_sweep.py` which has no such imports): `python path/to/script.py` sets `sys.path[0]` to that script's own directory, not the working directory, breaking the self-package import; `python -m package.module` sets it to the working directory instead, resolving correctly. Fixed by changing `scripts/dev.sh`'s `start_worker` to invoke every worker via `-m` with a dotted module path, including the two pre-existing sweep-worker call sites (for consistency) -- which in turn required updating `test_phase6.py`'s own stop/restart fixture to match the new process command line it greps for.

v20 changes from v19 (structural cleanup, no behavior change): catalog and theatre's `main.py` had grown into a single flat file mixing customer-facing and admin-facing routes, request bodies, and helpers -- confirmed with user: split each into `admin/`/`customer/`/`common/` (booking/payment/user untouched, since neither has any admin routes to separate out -- booking's are customer-facing + internal-only, payment/user have none at all). `common/` holds what's genuinely shared (the DB dependency, idempotency-key derivation, entity not-found lookups used by both sides); `customer/routes.py` holds the Appendix A browse-only handlers; theatre's admin side is further split by sub-domain (`theatres_screens.py`, `seat_layouts.py` + `lock.py` for the draft-lock helpers, `showtimes.py`, `schemas.py`) since it has 20+ admin endpoints across three clearly distinct concerns, while catalog's smaller admin surface stays in one `routes.py` + `schemas.py`. `main.py` in both is now a thin composition root: create the app, define `/health`, `include_router` the rest -- one process/port per service either way, route paths unchanged. One real gotcha: `admin/lock.py` needed `AUTH_ENABLED`, but importing it from `main.py` (the natural-seeming place, since that's where it was previously defined) would be circular -- `main.py` is what imports the admin routers. Fixed by moving `AUTH_ENABLED` into `common/config.py`, which both `main.py` and the routers import from independently. Full regression (55 passed/2 skipped) and both E2E suites (5/5, 7/7) re-verified passing with zero test changes -- route paths and behavior are identical, only which Python module defines each handler changed.

v21 changes from v20 (architectural correction to §5.7, raised by user review): the two-phase lock sequence's step 5 (`confirm_hold`) was being written to the Outbox *unconditionally* on every confirm -- purely async, no synchronous attempt at all. This re-opens exactly the race the Outbox was supposed to be a *fallback* for, not the routine path: a confirm landing near the tail of the hold's `hold_duration_seconds` window has no slack left, and the relay's poll interval (10s default) plus any backoff retries could easily outlast however much TTL the theatre's own system has left on that hold -- their side releasing it before our relay ever gets to it is a real cross-channel double-booking, undetectable from our side since we already show `CONFIRMED`. Fixed: `confirm()` now calls `theatre.confirm_hold()` *synchronously* first; the Outbox enqueue only happens if that synchronous attempt itself raises `TheatreIntegrationUnavailable` -- the common case (theatre API healthy) now closes the race entirely, with no relay involvement and no poll-interval delay at all. `release_hold` (cancel and sweep-expiry) deliberately keeps its purely-async Outbox-only behavior unchanged -- a cancel/expiry isn't time-critical against an external TTL the same way a *successful, paid-for* confirm is (the seat simply stays held slightly longer on the theatre's side if the release lags, not a double-booking risk), so the added complexity of a synchronous-first attempt there isn't justified. New failure handling for when the fallback Outbox path *also* exhausts its retries (§13, the automated half of "flag for manual reconciliation"): the relay now raises `TheatreConfirmationAbandoned` (caught immediately, logged with full traceback, so one abandoned confirmation never aborts the rest of that pass's batch) and publishes two new mock events through the existing `EventPublisher`/`LoggingEventPublisher` seam (§7/§15, the same no-op-until-a-real-bus pattern every other domain event in this codebase already uses) -- `TheatreConfirmationFailedEvent` (customer notification: their booking is confirmed on our side but we couldn't get the theatre to agree) and `RefundRequestedEvent` (since this system can no longer guarantee the seat is honored, the payment should be refunded rather than left standing). Both events live in `theatre_outbox_relay.py` next to the only code that raises them, matching where `BookingConfirmedEvent` already lives relative to its own sole caller. `THEATRE_MOCK_CONFIRM_HOLD_FAILS` (env var, mirrors `THEATRE_MOCK_HOLD_MODE`) lets a test force the synchronous attempt to fail against the live process, so the fallback path can be exercised end-to-end rather than only at the relay's own level.

v22 changes from v21 (structural cleanup, no behavior change): every service's configuration -- env-derived values and magic numbers alike (lock TTLs, retry/backoff/batch-size knobs, advisory lock keys, external service URLs, `AUTH_ENABLED`) -- had been scattered across whichever module happened to need it first. Confirmed with user: centralized into one `config.py` per service (`common/config.py` for catalog/theatre, reusing the v20 sub-package; a plain top-level `config.py` for booking/payment/user/routing/local-cdn-mock, none of which have that sub-package). Each is a pure leaf module -- only `import os`, nothing from elsewhere in its own service -- so anything in that service can import from it with zero circular-import risk, the same reasoning v20 already established for `AUTH_ENABLED`. The one real wrinkle: booking's three workers (`reconciliation_sweep.py`, `theatre_outbox_relay.py`, `theatre_availability_sync.py`) each had a constant named `DEFAULT_POLL_INTERVAL_SECONDS` (and similarly `DEFAULT_LOCK_RETRY_SECONDS`/`DEFAULT_BATCH_SIZE`) holding a *different* value per worker (20s/10s/60s) -- harmless while each lived in its own module's namespace, but a real collision once centralized into one shared module. Renamed with a per-worker prefix (`RECONCILIATION_*`/`OUTBOX_RELAY_*`/`AVAILABILITY_SYNC_*`) to resolve it. `CircuitBreaker`'s own constructor defaults (previously bare literals `3`/`30.0`) now reference the same centralized `CIRCUIT_BREAKER_FAILURE_THRESHOLD`/`CIRCUIT_BREAKER_RECOVERY_SECONDS`, which let `MockTheatreIntegration`'s own breaker construction drop its redundant explicit args entirely. Full regression (57 passed/2 skipped) and both E2E suites (5/5, 7/7) re-verified passing with zero test changes -- only which module defines each constant changed, not any value or behavior.

v23 changes from v22 (Phase 9.6, AI booking agent -- two build-time clarifications of an underspecified §2.1/Appendix A contract, confirmed with user before implementing rather than guessed): (1) **`find-seats`'s zone/centrality math assumed 0-1 normalized coordinates that don't exist anywhere in this system** -- `position_x`/`position_y` on `SEAT_TEMPLATE`/`SHOWTIME_SEAT` are raw layout units written by the admin canvas's placement tools (§4.5: a grid's rows land at integer y = 0, 1, 2, ...), not normalized to a 0-1 screen-relative range the way the front/middle/back thresholds (`<0.33`, `0.33-0.66`, `>0.66`) and centrality scoring (proximity to `x=0.5`) implicitly require. Resolved by normalizing both axes per-showtime: min-max scale against *every* materialized seat for that showtime (not just the currently-available ones, so the front/middle/back boundary doesn't drift as seats get booked) before classifying. (2) **"Adjacent," left undefined beyond "by position_x within the same zone," would have grouped seats from different rows** -- a zone is a horizontal band that can span several rows, and two seats in different rows can easily share a similar x without being physically side-by-side. Resolved by requiring an exact `position_y` match (same row) in addition to a close `position_x` before two seats count as adjacent. Implementation: `services/booking/application/seat_finder.py`, new `POST /showtimes/{id}/find-seats`. `services/agent-service/` (new service, :8007) and a `ChatWidget` in `apps/customer-web` round out the phase per Appendix B's build order -- routing's `SERVICE_MAP` gained an `"agent"` prefix entry to keep "frontends call through routing only" intact for the widget. Full regression (78 passed/2 skipped) re-verified passing with zero existing-test changes.
...
5. Confirm booking in Postgres          — mark_booked + mark_confirmed, release Redis lock
6. Call theatre.confirm_hold()         — external, synchronous, tells theatre to convert hold to a real booking
   - Success → done, nothing else to do
   - Failure → fall back to the Outbox (below) rather than failing the confirm itself —
     the booking is already correctly CONFIRMED and the customer already paid
```

Step 6 is deliberately **synchronous, not Outbox-only** (v21 correction) — a confirm landing near the tail of the hold's `hold_duration_seconds` window has no slack left; if this only ever went through the Outbox relay's poll interval (+ possible backoff retries), the theatre's own hold could expire and release the seat on their side before the relay ever got to it, a real cross-channel double-booking this system would have no way to even detect. Calling it inline closes that race in the common case (theatre API healthy) with zero relay involvement and zero poll-interval delay. The Outbox is the *fallback*, used only when the synchronous attempt itself fails — exactly the scenario it was built for (§13: a genuine downstream failure, not routine latency).

On expiry or cancellation, the sweep worker (§5.4) and the cancel endpoint (§5.6) additionally call `theatre.release_hold(theatre_hold_id)` to free the seat in the theatre's system — this path stays purely async (Outbox-only, no synchronous attempt), deliberately asymmetric with confirm_hold above: a release isn't time-critical against an external TTL the same way a successful, paid-for confirm is — if a release lags, the seat just stays held slightly longer on the theatre's side, not a double-booking risk. The Outbox itself is a `pending_theatre_call` table (`call_type`, `booking_id`, `theatre_hold_id`, `status`, `attempts`, `next_attempt_at`, `last_error`) written in the *same transaction* as whichever path triggered it, with `services/booking/adapters/theatre_outbox_relay.py` (same N-replicas-one-active pattern as the sweep worker, §5.4) calling the theatre API independently with bounded exponential backoff. A failure here never blocks or risks the booking/seat state itself -- the booking is already correctly CONFIRMED/CANCELLED/EXPIRED before the Outbox entry is even written. If a `CONFIRM_HOLD` entry itself exhausts its retry budget, the relay raises `TheatreConfirmationAbandoned` and publishes `TheatreConfirmationFailedEvent` (customer notification) and `RefundRequestedEvent` — the automated half of "flag for manual reconciliation": this system can no longer guarantee the theatre honors a booking it already took payment for, so the customer is notified and the payment is queued for refund (both currently mock events through the same `EventPublisher` seam as everything else, §7/§15).
...
class BookingOrchestrator:
    ...
    def confirm(self, booking_id: str, payment_id: str) -> Booking:
        ...
        # confirm_hold is called synchronously first (v21) -- closes the race
        # where a confirm near the tail of the hold's TTL could otherwise lose
        # to the theatre's own hold expiring before an Outbox relay got to it.
        # Only falls back to the Outbox if this call itself fails — see §5.7
        # and §13 for the failure case, including what happens if the fallback
        # also exhausts its retries.
        try:
            self._theatre.confirm_hold(booking.theatre_hold_id)
        except TheatreIntegrationUnavailable:
            self._outbox.enqueue("CONFIRM_HOLD", booking.id, booking.theatre_hold_id)
...
| Theatre API fails during the *synchronous* confirm_hold attempt (v21) | Booking is confirmed in BookMyShow but the theatre system may not know yet | Falls back to the Outbox (write the pending confirm-hold call in the same transaction as the booking confirm, a relay retries it independently with bounded backoff). |
| Theatre API still fails after the Outbox relay exhausts its retry budget | Theatre system may never learn to honor a booking already paid for — real double-booking risk on their side | Flag for manual reconciliation (`pending_theatre_call.status = 'FAILED'`) — an ops concern, not a data-integrity one, since BookMyShow has the payment. The relay also raises `TheatreConfirmationAbandoned` and publishes `TheatreConfirmationFailedEvent` (customer notification) + `RefundRequestedEvent` (mock events, same `EventPublisher` seam as everything else) so the customer is told and the payment is queued for refund rather than left standing. |
...
POST   /showtimes/{showtime_id}/find-seats                 { count, preferences: { adjacent, zone, seat_type } } → [{ seats, description, total_price, zone }] -- up to 3 ranked groups; agent-service only, not called by the customer SPA directly
...
├── services/
│   ├── catalog/                  (v20: admin/customer/common split -- see below)
│   │   ├── admin/                 (/admin/* routes -- routes.py + schemas.py)
│   │   ├── customer/              (Appendix A browse routes -- routes.py)
│   │   └── common/                (db.py, idempotency-key derivation, MOVIE lookups -- shared by both)
│   ├── theatre/                   (v20: same split, admin/ further divided by sub-domain since it's larger)
│   │   ├── admin/                 (theatres_screens.py, seat_layouts.py + lock.py, showtimes.py, schemas.py -- draft-lock enforcement, §4.6)
│   │   ├── customer/              (Appendix A browse routes -- routes.py)
│   │   └── common/                (db.py, config.py, idempotency-key derivation, THEATRE lookups)
│   ├── booking/
│   │   ├── domain/
│   │   ├── application/
│   │   ├── adapters/              (RedisSeatLocker incl. Lua script, reconciliation worker with advisory-lock leader election, PostgresBookingRepository, MockTheatreIntegration, theatre_outbox_relay + theatre_availability_sync workers, v18/v19)
...
├── agent-service/                     (AI booking agent — state machine + Ollama NLU; see docs/ai-agent-requirements.md)
