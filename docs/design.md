...
On expiry or cancellation, the sweep worker (§5.4) and the cancel endpoint (§5.6) additionally call `theatre.release_hold(theatre_hold_id)` to free the seat in the theatre's system. Neither calls it synchronously inline: both steps 5 (`confirm_hold`) and this release call go through a real Outbox -- a `pending_theatre_call` table (`call_type`, `booking_id`, `theatre_hold_id`, `status`, `attempts`, `next_attempt_at`, `last_error`) written in the *same transaction* as the booking confirm/cancel/sweep-expiry, with `services/booking/adapters/theatre_outbox_relay.py` (same N-replicas-one-active pattern as the sweep worker, §5.4) calling the theatre API independently with bounded exponential backoff. A failure here never blocks or risks the booking/seat state itself -- the booking is already correctly CONFIRMED/CANCELLED/EXPIRED before the Outbox entry is even written.
