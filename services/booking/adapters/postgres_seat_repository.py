...
class PostgresSeatRepository:
    ...
    def get_available_for_booking(self, showtime_id: str, seat_ids: list[str]) -> dict[str, dict]:
        ...
        checked here for a clean error; *availability* is enforced by the
        conditional UPDATE in lock_seats below, not by this read."""
        with self._conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM showtime_seat WHERE showtime_id = %s AND id::text = ANY(%s)",
    ...
    def lock_seats(self, showtime_id: str, seat_ids: list[str], booking_id: str, lock_expires_at) -> int:
        ...
        the actual backstop") -- AVAILABLE -> LOCKED, conditional on still
        being AVAILABLE. Returns the number of seats actually flipped;
        ...
        with self._conn.cursor() as cur:
            cur.execute(
                ...
                SET status = 'LOCKED', locked_by_booking_id = %(booking_id)s, lock_expires_at = %(lock_expires_at)s,
                    updated_at = now()
                WHERE showtime_id = %(showtime_id)s AND id::text = ANY(%(seat_ids)s) AND status = 'AVAILABLE'
                RETURNING id
                """,
                {
    ...
    def mark_booked(self, showtime_id: str, booking_id: str) -> list[str]:
        ...
        from locked_by_booking_id), conditional on still being LOCKED and
        not yet hold-expired. Returns the seat IDs actually flipped to
        BOOKED -- empty means either a racing confirm already won, or the
        hold expired; the caller (orchestrator) disambiguates."""
        with self._conn.cursor() as cur:
            cur.execute(
                """
                UPDATE showtime_seat
